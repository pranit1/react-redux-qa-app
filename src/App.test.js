...
test("Should create a Q/A and test posted Q/A", () => {
  ...
  userEvent.type(
    screen.getByTestId("input-question"),
...
test("Should remove questions", () => {
  ...
  expect(
    screen.queryByText(/Can i add my own questions/i)
...
test("Should sort questions", () => {
  ...
  expect(
    within(posts[0]).queryByText(/Can i add my own questions/i)
  ).toBeInTheDocument();
  expect(
    within(posts[1]).queryByText(/How to add a question/i)
  ).toBeInTheDocument();
  expect(
    within(posts[2]).queryByText(/A test for sorting/i)
  ).toBeInTheDocument();
  userEvent.click(screen.getByTestId("sort-questions-button"));
  posts = screen.queryAllByTestId("post-article");
  expect(
    within(posts[0]).queryByText(/A test for sorting/i)
  ).toBeInTheDocument();
  expect(
    within(posts[1]).queryByText(/Can i add my own questions/i)
  ).toBeInTheDocument();
  expect(
    within(posts[2]).queryByText(/How to add a question/i)
  ).toBeInTheDocument();
});
...
test("Should edit a question", async () => {
  ...
  userEvent.type(
    screen.queryByTestId("edit-question"),
    "Can I edit my own question"
  );
  userEvent.type(
    screen.queryByTestId("edit-answer"),
    "Yes you can edit your own question"
  );
